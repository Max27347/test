...
def load_image_from_file(file_path, size):
    image = pygame.image.load(file_path).convert_alpha()  # Используем convert_alpha() для поддержки прозрачности
    return pygame.transform.scale(image, size)  # Изменяем размер изображения

# Класс Игрока
class Player:
    def __init__(self):
        self.image = load_image_from_file('/static/images/mortis.png', (50, 50))
...
class Bomb:
    def __init__(self):
        self.image = load_image_from_file('/static/images/dyno.png', (30, 30))
        self.rect = self.image.get_rect()
        self.reset()
    ...
    def increase_speed(self):
        if self.speed < 15:  # Максимальная скорость (можно настроить)
            self.speed += 0.01  # Увеличиваем скорость
